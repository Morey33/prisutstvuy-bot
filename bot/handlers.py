...
MENU_BUTTONS = {"Как я сейчас", "Утреннее намерение", "Вечерняя пауза", "Практика", "Получить подписку", "Настройки"}
...
async def handle_state_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "Как я сейчас":
        await update.message.reply_text("Расскажи — как ты прямо сейчас?\n\nМожно одним словом, можно больше. Напиши как есть.", reply_markup=ReplyKeyboardRemove())
    ...
    if text == "Вечерняя пауза":
        await update.message.reply_text("Один вопрос на сегодня:\n\nГде ты был по-настоящему здесь?\n\nНапиши — один момент, одно слово, образ. Всё подойдёт.", reply_markup=ReplyKeyboardRemove())
        context.user_data["awaiting_evening"] = True
        return

    if text == "Практика":
        last = context.user_data.get("last_state", "anxiety")
        last_practice = context.user_data.get("last_practice")
        practice = get_practice(last, exclude=last_practice)
    ...
    if context.user_data.get("awaiting_state") or text not in MENU_BUTTONS:
        context.user_data["awaiting_state"] = False
...
async def grant(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Открыто для {target_id}")
    await context.bot.send_message(chat_id=target_id, text="Твой доступ открыт.\n\nПросто начни — никаких обязательств.")
